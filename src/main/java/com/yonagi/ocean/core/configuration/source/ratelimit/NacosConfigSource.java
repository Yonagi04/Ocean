...
public class NacosConfigSource implements ConfigSource {
    ...
    public List<RateLimitConfig> load() {
        try {
            ...
            log.info("Loaded {} rate limit configurations from Nacos configuration", rateLimitConfigs.size());
            ...
            log.error("Failed to load rate limit configuration from Nacos: {}", e.getMessage(), e);
            return null;
        }
    }
